...
import csv
import re
targetFolder = "cd ../CR-group-1/commons-math/;"
...
def findIssueID():
    cmd = targetFolder + 'git log --before="2017-11-01" --name-status --reverse --all --date=iso --grep="MATH-"'
    commits = []
    IssueID =[]
    newCommitandIDs = []
    for char in cmdline(cmd):
    ...
    for i in xrange(len(commits)):
        match = re.findall('MATH-\d+',commits[i])
        # print match
    ...
    for commit in commits:
            match = re.findall('MATH-\d+',commit)
            for ID in IssueIDs:
...
def writeTocsv():
    with open('resultMath.csv', 'wb') as csvfile:
